...
=== Github-Übung mit Professor Aberger
...
Dann die File id_rsa.pub die soeben erstellt wurde bei github unter settings > ssh und gpg keys > neuen ssh key erstellen und den inhalt der file dann einfügen
...
als group die umgekehrte firmen domäne eingeben in diesem Fall at.htlleonding angeben
...
git stash
----
die momentane arbeit abgelegt werden um sie nachher mit
[source, bash]
----
git stash pop
----
...
Wenn das Feature fertig gebaut wurde, muss der Branch wieder mit dem main branch gemerged werden dies kann durch manuelles mergen oder durch eine pull request stattfinden.